...
const teams = schools.flatMap(([schoolKey, name, providerId]) => sports.flatMap(([sportKey, sport]) => levels.map(([levelKey, level]) => ({
  id: `OLATHE:${schoolKey}:${sportKey === 'BOYS_SOCCER' ? 'SOCCER' : sportKey}:${levelKey}`,
  providerId,
  league: 'OLATHE',
  schoolKey,
  sportKey,
  ...
  levelKey,
  level,
  scoresAvailable: levelKey === 'VARSITY' && ((schoolKey === 'ONW' && sportKey === 'FOOTBALL') || sportKey === 'BOYS_SOCCER'),
  name,
...
export default {
  ...
  games: async date => {
    ...
    const requests = [endpoint(`${workerBase}/api/olathe/scores?date=${current}`), endpoint(`${workerBase}/api/olathe/scores?date=${current}&sport=soccer`)];
    if (typeof window !== 'undefined' && window.location.hash === '#home') {
      const previous = new Date(date);
      previous.setUTCDate(previous.getUTCDate() - 1);
      dates.push(isoDate(previous));
      requests.push(endpoint(`${workerBase}/api/olathe/scores?date=${isoDate(previous).replaceAll('-', '')}`), endpoint(`${workerBase}/api/olathe/scores?date=${isoDate(previous).replaceAll('-', '')}&sport=soccer`));
    }
    ...
    return games.map(game => {
      ...
      return { ...game, sport: game.sport || 'FOOTBALL', sportName: game.sportName || (game.sport === 'SOCCER' ? 'Soccer' : 'Football'), homeTeam: mapTeam(game.homeTeam), awayTeam: mapTeam(game.awayTeam), school: mapTeam(game.school) };
